...
def get_corpdata(conn, edinet_code, message_ref=None):
    ...
    if row is None:
        ...
        return fetch_and_save_corpdata(conn, edinet_code, message_ref)

    # 最終確認日時
    updated_at = datetime.fromisoformat(row[-1])

    # 7日以内ならDBをそのまま使用
    if datetime.now(UTC) - updated_at < CHECK_INTERVAL:
        return row

    # 7日以上経過
    # EDINET DBに新しい決算があるか確認
    return refresh_corpdata_if_needed(conn, edinet_code, message_ref)
...
def fetch_and_save_corpdata(conn, edinet_code, message_ref=None):
    ...
    try:
        return save_corpdata(conn, edinet_code, result, message_ref)
...
def refresh_corpdata_if_needed(conn, edinet_code, message_ref=None):
    ...
    new_disclosure_date = earnings.get("disclosure_date")
    ...
    if new_disclosure_date == old_disclosure_date:
        conn.execute(
            ...
            WHERE EDINETCode = ?
        """,
            (datetime.now(UTC).isoformat(timespec="seconds"), edinet_code),
        )
...
def save_corpdata(conn, edinet_code, result, message_ref=None):
    ...
    pbr = None
    if per is not None and eps is not None and bps is not None and bps != 0:
        pbr = per * eps / bps
    # ROE
    ...
    if (
        ...
        if implied_price > 0:
            dividend_yield = dividend_per_share / implied_price
    ...
    if interest_bearing_debt is None:
        ...
        if any(value is not None for value in debt_fields):
            interest_bearing_debt = sum(value or 0 for value in debt_fields)
        else:
            interest_bearing_debt = None

    # その他
    land = financials.get("land")
    investment_securities = financials.get("investment_securities")
    # 更新日時
