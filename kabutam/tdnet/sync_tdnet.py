...
def sync_recent_tdnet(conn, message_ref=None):
    ...
    for index, date_str in enumerate(target_dates):
        ...
        if index < TDNET_RECENT_DAYS:
            if not should_sync_tdnet(
                conn, date_str, interval_hours=TDNET_SYNC_INTERVAL_HOURS
            ):
                continue
    ...
    # 古い同期情報削除
    cleanup_tdnet_sync_status(conn, keep_days=TDNET_SYNC_STATUS_RETENTION_DAYS)
...
def should_sync_tdnet(conn, date_str, interval_hours=TDNET_SYNC_INTERVAL_HOURS):
    ...
    last_checked = datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S").replace(tzinfo=UTC)
...
def sync_tdnet_doc_list(conn, date_str=None, message_ref=None, completed=False):
    ...
    if message_ref is not None:
        message_ref[0] = f"TDnet [{date_str}] の開示情報を取得しています..."

    documents = search_tdnet_doc_list_data(date_str)

    if documents is None:
        if message_ref is not None:
            message_ref[0] = f"TDnet [{date_str}] の取得に失敗しました。"
        return 0
    ...
    if not documents:
        ...
        if message_ref is not None:
            message_ref[0] = f"TDnet [{date_str}] の開示情報はありませんでした。"
        return 0
    ...
    if message_ref is not None:
        message_ref[0] = f"TDnet [{date_str}] {saved_count}件の新規開示を保存しました。"
...
def save_tdnet_doc_list(conn, documents):
    ...
    for doc in documents:
        ...
        try:
            ...
            print(f"Error saving TDnet document {pdf_url}: {e}")
